...
Alumnos: Barragan Pilar Diana			318147981
...
float giroBrazo = 0.0, giraMano = 0.0, giraDedos = 0.0;
float giroBrazoOffset = 0.3, giraManoOffset = 0.3, giraDedosOffset = 0.3;
bool mueveBrazo = true, mueveMano = false, mueveBrazo2 = false;
...
//NEW// Keyframes
float posXavion = 0.0, posYavion = 50.0, posZavion = 0.0;
...
typedef struct _frame {
	...
	float movAvion_z;		//NEW
	float movAvion_xInc;	//Variable para IncrementoX
	float movAvion_yInc;	//Variable para IncrementoY
	float movAvion_zInc;	//NEW
	float giroAvion;
	float giroAvionInc;
}FRAME;
...
void saveKeyframesToFile(const char* filename) {
	...
	outFile << FrameIndex << std::endl; // Guardar número total de frames

	for (int i = 0; i < FrameIndex; i++) {
		outFile << KeyFrame[i].movAvion_x << " " << KeyFrame[i].movAvion_y << " "
			<< KeyFrame[i].movAvion_z << " " << KeyFrame[i].giroAvion << std::endl;
	}
	outFile.close(); printf("Keyframes guardados exitosamente en %s\n", filename);
}
...
void loadKeyframesFromFile(const char* filename) {
	...
	for (int i = 0; i < FrameIndex && i < MAX_FRAMES; i++) {
		inFile >> KeyFrame[i].movAvion_x >> KeyFrame[i].movAvion_y
			>> KeyFrame[i].movAvion_z >> KeyFrame[i].giroAvion;
	}
	inFile.close();
	printf("Keyframes cargados exitosamente desde %s\n", filename);
}

void inputKeyframesFromConsole() {
	char continuar; bool inputting = true;
	while (inputting && FrameIndex < MAX_FRAMES) {
		printf("\n=== Ingresando Keyframe %d ===\n", FrameIndex);

		printf("Posición X: "); std::cin >> KeyFrame[FrameIndex].movAvion_x;
		printf("Posición Y: "); std::cin >> KeyFrame[FrameIndex].movAvion_y;
		printf("Posición Z: "); std::cin >> KeyFrame[FrameIndex].movAvion_z;
		printf("Giro: "); std::cin >> KeyFrame[FrameIndex].giroAvion;
		FrameIndex++;

		if (FrameIndex >= MAX_FRAMES) {
			printf("\nYA SON MUCHOS KEYFRAMES, LIMITE ALCANZADO\n", MAX_FRAMES);
			break;
		}

		printf("\n¿Deseas ingresar otro keyframe? (s/n): ");
		std::cin >> continuar;
		std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

		if (tolower(continuar) != 's') { inputting = false; }
	}

	printf("\nTotal de keyframes ingresados: %d\n", FrameIndex);

	printf("¿Deseas guardar los keyframes en archivo? (s/n): ");
	std::cin >> continuar;
	if (tolower(continuar) == 's') { saveKeyframesToFile("keyframes.txt"); }
}
...
void showKeyframes() {
	...
	for (int i = 0; i < FrameIndex; i++) {
		printf("Keyframe %d:\n", i);
		printf("X: %.2f, Y: %.2f, Z: %.2f, Giro: %.2f\n",
			KeyFrame[i].movAvion_x, KeyFrame[i].movAvion_y,
			KeyFrame[i].movAvion_z, KeyFrame[i].giroAvion);
	}
	printf("Total de keyframes: %d\n", FrameIndex);
}
...
void animate(void) { //Movimiento del objeto con barra espaciadora
	if (play) {
		if (i_curr_steps >= i_max_steps) {//fin de animación entre frames?
			playIndex++;
			printf("playindex : %d\n", playIndex);
			if (playIndex > FrameIndex - 1) {//Fin de toda la animación con último frame?
...
int main() {
	...
	DedoDerechoBaymax = Model();		DedoDerechoBaymax.LoadModel("Models/BaymaxBlanco/DedoDerecho.obj");
	DedoGordoBaymax = Model();			DedoGordoBaymax.LoadModel("Models/BaymaxBlanco/DedoGordo.obj");
	DedoIzquierdoBaymax = Model();		DedoIzquierdoBaymax.LoadModel("Models/BaymaxBlanco/DedoIzquierdo.obj");
	DedoMedioBaymax = Model();			DedoMedioBaymax.LoadModel("Models/BaymaxBlanco/DedoMedio.obj");
	...
	Edificio5 = Model();
	Edificio5.LoadModel("Models/edificio5.obj");
	Edificio6 = Model();
	Edificio6.LoadModel("Models/edificio6.obj");

	//Phineas and Ferb
	Casa2 = Model();
	Casa2.LoadModel("Models/casa2.obj");
	Doof = Model();
	Doof.LoadModel("Models/doof.obj");
	Edificio7 = Model();
	Edificio7.LoadModel("Models/edificio7.obj");
	Edificio8 = Model();
	Edificio8.LoadModel("Models/edificio8.obj");
	Edificio9 = Model();
	Edificio9.LoadModel("Models/edificio9.obj");
	Edificio10 = Model();
	Edificio10.LoadModel("Models/edificio10.obj");

	//BIG HERO SIX
	Edificio15 = Model();
	Edificio15.LoadModel("Models/edificio15.obj");
	Edificio16 = Model();
	Edificio16.LoadModel("Models/edificio16.obj");
	Edificio17 = Model();
	Edificio17.LoadModel("Models/edificio17.obj");
	Edificio18 = Model();
	Edificio18.LoadModel("Models/edificio18.obj");
	Torre = Model();
	Torre.LoadModel("Models/torre.obj");
	Puente = Model();
	Puente.LoadModel("Models/puente.obj");

	Hovercar = Model(); Hovercar.LoadModel("Models/Hovercar.obj");
	...
	printf("3.-Presionar I para ingresar keyframes.\n4.-Presionar G para guardar keyframes en archivo\n");
	printf("5.-Presionar C para cargar keyframes desde archivo.\n6.-Presionar V para ver keyframes actuales\n");
	...
	while (!mainWindow.getShouldClose()) {
		...
		//Animacion BAYMAX "SALUDO"
		if (mueveBrazo) {
			if (giroBrazo < 85 or giroBrazo > 90) {
				giroBrazo += giroBrazoOffset * deltaTime;
			}
			else { mueveBrazo = !mueveBrazo; mueveMano = true; }
			if (giroBrazo > 180) { giroBrazo = 0.0; giraDedos = 0.0; giraMano = 0.0; }
		}
		else {
			if (mueveMano) {
				if (giraMano < 90) { giraMano += giraManoOffset * deltaTime; }
				else {
					giroBrazo = 91; mueveMano = -!mueveMano;
					giraDedos += 0.5f * deltaTime; mueveBrazo = true;
				}
			}
		}
		...
		if (FASE_01) {
			LightDAY.SetDirection(xDir, -0.25f, 0.0f/*zDir*/);
			shaderList[0].SetDirectionalLight(&LightDAY);
			skyboxDia.DrawSkybox(camera.calculateViewMatrix(), projection);
		}
		else {
			LightNIGHT.SetDirection(xDir, -0.25f, 0.0f/*zDir*/);
		...
		glm::mat4 modelLamp1(1.0);
		glm::mat4 modelLamp2(1.0);
		glm::mat4 modelLamp3(1.0);
		glm::mat4 modelLamp4(1.0);
		glm::mat4 modelauxBaymax(1.0);
		...
		if (PerPrincipal1 == true) { 
			...
			model = glm::mat4(1.0); // Baymax Blanco Con Jerarquía
			model = glm::translate(model, glm::vec3(movX, 0.0f, movZ));
			model = glm::rotate(model, glm::radians(giro), glm::vec3(0.0f, 1.0f, 0.0f));
			...
			model = modelaux2;
			model = glm::translate(model, glm::vec3(-13.0f, -30.0f, 0.0f));
			modelaux2 = model;
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			...
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoMedioBaymax.RenderModel();

			model = modelaux2;
			model = glm::translate(model, glm::vec3(-5.0f, -4.0f, 2.0f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoIzquierdoBaymax.RenderModel();

			model = modelaux2;
			model = glm::translate(model, glm::vec3(-5.0f, -4.0f, -2.0f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoDerechoBaymax.RenderModel();

			model = modelaux2;
			model = glm::translate(model, glm::vec3(2.0f, -4.0f, 2.0f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoGordoBaymax.RenderModel();
		}
		...
		if (PerPrincipal1 == true) {
			...
			model = modelauxBaymax;
			model = glm::translate(model, glm::vec3(-13.0f, -30.0f, 0.0f));
			model = glm::rotate(model, giraMano * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
			modelauxBaymax = model;
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			ManoBaymax.RenderModel();
			...
			model = glm::rotate(model, sin(giraDedos) * toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoMedioBaymax.RenderModel();
			...
			model = modelauxBaymax;
			model = glm::translate(model, glm::vec3(2.0f, -4.0f, 2.0f));
			model = glm::rotate(model, sin(giraDedos) * toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			DedoGordoBaymax.RenderModel();
		}
		else if (PerPrincipal3 == true) {
		...
		model = glm::rotate(model, (-90 + giroAvion) * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Hovercar.RenderModel();
...
void inputKeyframes(bool* keys) {
	...
	if (keys[GLFW_KEY_SPACE]) {// Tecla ESPACIO - Reproduce animacion
		if (reproduciranimacion < 1) {
			if (play == false && (FrameIndex > 1)) {
				resetElements(); interpolation();
				play = true; playIndex = 0; i_curr_steps = 0;
				reproduciranimacion++;
				printf("\n Tecla 0 para habilitar reproducion de animación nuevamente\n");
	...
	if (keys[GLFW_KEY_0]) {// Tecla 0 - Cargar Animacion de nuevo
	...
	if (keys[GLFW_KEY_I]) { // Tecla I - Ingresar KEYFRAMES
		if (!iKeyPressed) { iKeyPressed = true; inputKeyframesFromConsole(); }
	}
	else { iKeyPressed = false; }

	if (keys[GLFW_KEY_G]) { // Tecla G - Guardar KEYFRAMES
		if (!gKeyPressed) { gKeyPressed = true; saveKeyframesToFile("keyframes.txt"); }
	}
	else { gKeyPressed = false; }

	if (keys[GLFW_KEY_C]) { // Tecla C - Cargar KEYFRAMES
		if (!cKeyPressed) { cKeyPressed = true; loadKeyframesFromFile("keyframes.txt"); }
	}
	else { cKeyPressed = false; }

	if (keys[GLFW_KEY_V]) { // Tecla V - Ver KEYFRAMES actuales
		if (!vKeyPressed) { vKeyPressed = true; showKeyframes(); }
	}
